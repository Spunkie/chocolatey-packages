...
function Update-IconUrl{
  ...
  if($UsePackagesDirectoryForIconDir) {
      $resolvedPath = "$RelativeIconDir/$iconNameWithExtension"
  } else {
...
If ($Name) {
    if($UsePackagesDirectoryForIconDir) {
        $RelativeIconDir = "$PackagesDirectory/$Name"
    }
  ...
  foreach ($directory in $directories) {
    if ((Test-Path "$($directory.FullName)/$($directory.Name).nuspec")) {
        if($UsePackagesDirectoryForIconDir) {
            $RelativeIconDir = "$PackagesDirectory/$directory/$Name"
            $RelativeIconDir = $RelativeIconDir.TrimEnd("/");
        }
